...
| 6 | **Relevancia producción software** (80) | **0,03052** | **0,02793** | **0,02524** | **0,07602** | **0,02467** | 2025 Q4 | GitHub Innovation Graph | ✅ Determinístico · ALTA |
...
- **Valores (2025 Q4):** ES **0,03052** · EE **0,02793** · SG **0,02524** (0,02351 sin agregado "EU") · DE **0,07602** · PT **0,02467**.

  | País | Repos | Inbounds recibidos | Relevancia |
  |---|---|---|---|
  | ES | 5.805.695 | 177.166 | 0,03052 |
  | EE | 416.635 | 11.635 | 0,02793 |
  | SG | 6.719.641 | 169.600 | 0,02524 |
  | DE | 9.976.837 | 758.450 | 0,07602 |
  | PT | 1.542.206 | 38.048 | 0,02467 |
- **Método (determinístico):** descarga de `repositories.csv` y `economy_collaborators.csv` del GitHub Innovation Graph (CC0). El datasheet define collaborators como *git pushes + PRs de un dev [source] a un repo de [destination]* → **inbound recibido = Σ weight con destination = país**. Sin auto-bucles. Script `github_relevancia_sw.py` reproduce todo.
- **Confianza:** ALTA (dato crudo oficial). Reserva: confirmar que la agregación (trimestre vs año) coincide con el cálculo ILIA 2025.
